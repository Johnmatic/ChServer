...
#define USE_CUP_WEAPONS_IN_CRATES 1
...
/* Mission System Settings */
	...
	DMS_TimeToFirstMission				= [60,180];				// [Minimum,Maximum] time between first mission spawn. | DEFAULT: 3-7 minutes.
	DMS_TimeBetweenMissions				= [180,360];				// [Minimum,Maximum] time between missions (if mission limit is not reached) | DEFAULT: 10-15 mins
	...
	DMS_RandomBanditMissionsOnStart		= 1;						// Number of (random) bandit missions to spawn when the server starts, just so players don't have to wait for missions to spawn.
...
/* DonkeyPunchDMS Custom Side Factions */
	...
	DMS_AIDistanceCheckFrequency		= 120;						// How often to check within DMS_fnc_TargetsKilled whether or not the AI is out of the maximum radius. Lower values increase frequency and increase server load, greater values decrease frequency and may cause longer delays for "runaway" AI.
...
/* Loot Settings */
	...
	DMS_CrateCase_Sniper =				[							// If you pass "Sniper" in _lootValues, then it will spawn these weapons/items/backpacks
											[
												#ifdef USE_CUP_WEAPONS_IN_CRATES
												["CUP_srifle_AS50",1],
